...
export default function Home() {
  ...
  const handleShare = async () => {
    ...
    const url = `${window.location.origin}${window.location.pathname}#${currentCard.id}`
  ...
  // Sync current index with scroll and update URL hash
  useEffect(() => {
    const container = containerRef.current
    if (!container) return
  ...
  useEffect(() => {
    ...
    if (hash) {
      const cardIndex = cards.findIndex((card) => card.id === hash)
      if (cardIndex !== -1) {
        // Small delay to ensure container is ready
        setTimeout(() => {
          scrollToSection(cardIndex)
        }, 100)
      }
    }
  }, [scrollToSection])
